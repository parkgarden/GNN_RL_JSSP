    ...
    while True:
        ...
        with torch.no_grad():
            ...
            while True:
                ...
                if pi.squeeze()[indices[-1-idx]] <= pi.squeeze()[indices[-1-math.ceil(qt_0_75)]] or idx >= qt_0_75:
                # if idx >= qt_0_75:
...
file_writing_obj = open('./' + 'drltime_'  + '_' + str(N_JOBS_N) + 'x' + str(N_MACHINES_N) + '_' + str(N_JOBS_P) + 'x' + str(N_MACHINES_P) + '_batch1_jp_WoSo_v0.5.txt', 'w')
...
np.save('drlResult_' + '_' + str(N_JOBS_N) + 'x' + str(N_MACHINES_N) + '_' + str(N_JOBS_P) + 'x' + str(N_MACHINES_P) + '_batch1_jp_WoSo_v0.5', np.array(result, dtype=np.single))
