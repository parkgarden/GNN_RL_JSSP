...
from models.actor_critic_GAT import ActorCritic
...
class PPO:
    ...
    def update(self, memories, n_tasks, g_pool):
        ...
        mb_g_pool = g_pool_cal(g_pool, torch.Size([x // 2  for x in torch.stack(memories[0].adj_mb).to(device).shape]), n_tasks, device)
...
def main():
    ...
    for i_update in range(configs.max_updates):
        ...
        if (i_update + 1) % 100 == 0:
            file_writing_obj = open('./' + 'log_' + str(configs.n_j) + '_' + str(configs.n_m) + '_' + str(configs.low) + '_' + str(configs.high) + '_jp.txt', 'w')
        ...
        if (i_update + 1) % 100 == 0:
            ...
            file_writing_obj1 = open(
                './' + 'vali_' + str(configs.n_j) + '_' + str(configs.n_m) + '_' + str(configs.low) + '_' + str(configs.high) + '_jp.txt', 'w')
            file_writing_obj1.write(str(validation_log))
        t5 = time.time()

        # print('Training:', t4 - t3)
        # print('Validation:', t5 - 4)
