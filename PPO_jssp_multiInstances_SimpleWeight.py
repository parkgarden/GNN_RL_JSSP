...
from validation_SW import validate
import math
...
def main(w_j, w_m):
    ...
    for i_update in range(configs.max_updates):
        ...
        for i, env in enumerate(envs):
            ...
            i1 = np.identity(len(adj))
            i2 = np.identity(len(adj) - 1)
            adj_j = np.pad(i2,[(1,0),(0,1)],mode = 'constant') + i1
            num_job = configs.n_j
            job_idx=1
            while(True):
                if job_idx*num_job >= len(adj):
                    break
                adj_j[job_idx*num_job][job_idx*num_job-1] = 0
                job_idx +=1
                        
            adj = adj_j * w_j + (adj - adj_j) * w_m
        ...
        t4 = time.time()
        if (i_update + 1) % 1000 == 0:
            vali_result = - validate(vali_data, ppo.policy, w_j, w_m).mean()
...
if __name__ == '__main__':
    ...
    for w_m in w_m_list:
        ...
        total2 = time.time()
        print(total2 - total1)
        cpu_times.append(total2 - total1)
